...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ['en']
        self.domains = ['dl.newmyvideolink.xyz','newmyvideolink.xyz', 'beta.myvideolinks.xyz', 'videolinks.ga', 'get.newmyvideolink.xyz']
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    links = re.findall('''(?:)((?:http)[^["'<]+)''', item)
                    for url in links:                        
                        try:
                            if any(x in url for x in ['.rar', '.zip', '.iso', 'youtube']): raise Exception()
