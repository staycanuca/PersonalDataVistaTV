...
if __name__ == '__main__':
    ...
    try:
        ADDON = xbmcaddon.Addon('script.tvguide.vistatv')
        ...
        if ADDON.getSetting('background.service') == 'true':
            ...
            xbmc.log("[script.tvguide.vistatv] Background service started...", xbmc.LOGDEBUG)
            Service()
            ...
            while not monitor.abortRequested():
                ...
                xbmc.log("[script.tvguide.vistatv] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[script.tvguide.vistatv] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[script.tvguide.vistatv] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
