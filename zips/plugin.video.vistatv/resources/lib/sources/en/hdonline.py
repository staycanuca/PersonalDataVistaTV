    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            try:
                ...
                r = json.loads(r)['html']
                #r = client.parseDOM(r, 'div', attrs = {'class': 'goblock-header'})
                ...
                r = [(i.attrs['data-id'], i.attrs['data-server'], dom_parser2.parse_dom(i.content, 'a', req='title')) for i in r]
                    ...
                    try:
                        ...
                        if (episode == 0) or (int(ep) == int(episode)):
                            ...
                            while length == 0 and count < 11:
                                ...
                                if length == 0: count += 1
                            url = []
                            url = re.findall('''file['"]:['"]([^'"]+)''', r)
                            url = [i.replace('\\','') for i in url if not i.endswith('.srt')]
                            for s in url:
                                try:
                                    if 'googleapis' in s:
                                        urls = directstream.googletag(s)
                                        if not urls:
                                                quality, info = info = source_utils.get_release_quality(s, eid[2])
                                                sources.append({'source': 'gvideo', 'quality': quality, 'language': 'en', 'url': s, 'direct': True, 'debridonly': False})
                                        else:
                                            for i in urls:
                                                sources.append({'source': 'gvideo', 'quality': i['quality'], 'language': 'en', 'url': i['url'], 'direct': True, 'debridonly': False})
                                    elif 'lh3.' in s:
                                        urls = directstream.googletag(s)
                                        for i in urls:
                                            try:
                                        ...
                                        quality, info = info = source_utils.get_release_quality(eid[2], eid[2])
